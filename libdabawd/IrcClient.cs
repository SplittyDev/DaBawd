...
namespace libdabawd
{
	public class IrcClient
	{
		...
		/// </summary>
		public delegate void ChannelMessageHandler (string channel, string message, string sender);
		...
		public IrcClient () {
			...
			ChannelMessage += (channel, message, sender) => { };
		...
		void _Listen () {
			...
			while (client.Connected && !Reader.EndOfStream) {
				...
				if (line != null) {
					...
					var com = (IrcCommand) string.Join (" ", commandParts);
					switch (com.Name) {
					...
					case "PRIVMSG":
						...
						if (com.Args.Length >= 2) {
							if (com.Args [0].StartsWith ("#"))
								ChannelMessage (com.Args [0], msg, sender_nick);
							else
